...
export function SearchPageClient({
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true)
        // Fetch properties using the server action, with error handling
        const { properties, error, debug } = await searchProperties(params)

        setProperties(properties || [])
        setError(error)
        setDebugInfo(debug)
        console.log(params)
        console.log("Properties received from server:", properties?.length || 0)
        ...
        setProperties([])
        setError("Došlo je do greške prilikom pretraživanja. Molimo pokušajte ponovno.")
        setDebugInfo(null)
      } finally {
        setIsLoading(false)
      }
  ...
  // Add this function to the SearchPageClient component to handle listing type changes
  const router = useRouter()
  const handleListingTypeChange = (type: "for_sale" | "for_rent" | undefined) => {
...
type Property = {
  ...
  bathrooms: number
  area: number
  property_type: string // Changed from type to property_type
  images: string[]
  featured?: boolean
  new?: boolean
  dateAdded: string
...
// Renamed from SearchResults to SearchResultsComponent to avoid naming conflict
function SearchResultsComponent({
  ...
  isLoading: boolean
  debugInfo?: any
  handleListingTypeChange: (type: "for_sale" | "for_rent" | undefined) => void
}) {
  const router = useRouter()
  ...
  // Update properties state when initialProperties changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (filters.propertyTypes.length > 0) {
      results = results.filter((property) => filters.propertyTypes.includes(property.property_type))
    }

    // Apply search query (client-side filtering for already fetched properties)
    if (searchQuery && !urlSearchParams.get("query")) {
  ...
  }, [filters, sortOption, searchQuery, properties]) // Remove urlSearchParams from dependencies
  ...
  const toggleCompare = (propertyId: string, e: React.MouseEvent) => {
    ...
    // This is already correct, but make sure it's there
  ...
  const getPlaceholderImage = (title: string) => {
    return `/placeholder.svg?height=400&width=600&text=${encodeURIComponent(title || "Nekretnina")}`
  }

  // Simple mock map data for future implementation
  const mockMapData = {
    center: { lat: 45.815, lng: 15.982 }, // Zagreb center
    zoom: 12,
  }
  ...
  const applyFilters = () => {
    ...
    // Update area range parameters
    params.set("minArea", filters.areaRange[0].toString())
    params.set("maxArea", filters.areaRange[1].toString())
    ...
    // Update property type parameter
    if (filters.propertyTypes.length > 0) {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-grow pt-20">
        <div className="w-full px-[21px]">
          ...
          <div className="bg-white rounded-lg shadow-md p-3 sm:p-4 mb-4 sm:mb-6 sticky top-[60px] sm:top-[80px] z-30">
                ...
                <div className="relative">
                  ...
                  {searchQuery && (
                    <button
                      ...
                      onClick={() => {
                        setSearchQuery("")
                        // If we're already using the query parameter, clear it by navigating
            ...
            {filtersOpen && (
              <div className="mt-4 pt-4 border-t border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFilters({
                              ...filters,
                              areaRange: [filters.areaRange[0], Number.parseInt(e.target.value) || 2000],
                            })
                          }
                          className="w-1/2"
                        />
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {["Stan", "Kuća", "Vila", "Apartman", "Penthouse", "Studio", "Zemljište"].map((type) => (
                ...
                {/* Listing type filter */}
                <div className="mt-4">
                  <h3 className="font-medium mb-3">Vrsta oglasa</h3>
                  <div className="flex gap-2">
                    <Button
                      variant={!urlSearchParams.get("status") ? "default" : "outline"}
                      className={cn(!urlSearchParams.get("status") && "bg-rose-500 hover:bg-rose-600")}
                      onClick={() => handleListingTypeChange(undefined)}
                    >
                      Sve
                    </Button>
                    <Button
                      variant={urlSearchParams.get("status") === "for_sale" ? "default" : "outline"}
                      className={cn(urlSearchParams.get("status") === "for_sale" && "bg-rose-500 hover:bg-rose-600")}
                      onClick={() => handleListingTypeChange("for_sale")}
                    >
                      Prodaja
                    </Button>
                    <Button
                      variant={urlSearchParams.get("status") === "for_rent" ? "default" : "outline"}
                      className={cn(urlSearchParams.get("status") === "for_rent" && "bg-rose-500 hover:bg-rose-600")}
                ...
                <div className="mt-6 flex justify-end gap-2">
                  ...
                  <Button className="bg-rose-400 hover:bg-rose-500" onClick={applyFilters}>
                    Primijeni filtere
                  </Button>
                </div>
              </div>
          ...
          {showDebug && debugInfo && (
            <div className="bg-gray-100 p-4 rounded-lg mb-6 text-xs font-mono overflow-auto max-h-[300px]">
            ...
            <>
              ...
              {viewMode === "map" ? (
                <div className="h-[600px] mb-6">
                  {/* Fallback in case the map can't load */}
                  <div className="bg-gray-100 rounded-lg h-full w-full flex items-center justify-center">
                    <div className="text-center p-6">
                      ...
                      <h3 className="text-xl font-semibold mb-2">Karta trenutno nije dostupna</h3>
                      <p className="text-gray-600 max-w-md mx-auto">
                        Pokušajte koristiti prikaz liste ili mreže za pregled nekretnina.
                      </p>
                    </div>
                  </div>
                </div>
                      ...
                      <div className="absolute inset-0 bg-[#e8e8e8]">
                        <div className="h-full w-full relative">
                          ...
                          {currentResults.map((property, index) => {
                            ...
                            const idSum = String(property.id)
                              .split("")
...
// Update the PropertyCard component props to use property_type instead of type
