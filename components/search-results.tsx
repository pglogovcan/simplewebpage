...
import { addToCompare, removeFromCompare, clearCompareProperties } from "@/lib/compare-store"
...
export function SearchResults({ initialProperties, searchParams, error }: SearchResultsProps) {
  ...
  const [filters, setFilters] = useState<FilterState>({
    priceRange: [
      ...
      searchParams.maxPrice ? Number(searchParams.maxPrice) : 1000000,
    ],
    areaRange: [
      searchParams.minArea ? Number(searchParams.minArea) : 0,
      searchParams.maxArea ? Number(searchParams.maxArea) : 500,
    ],
    bedrooms: searchParams.bedrooms ? [Number(searchParams.bedrooms)] : [],
    propertyTypes: searchParams.propertyType ? [searchParams.propertyType as string] : [],
    amenities: [],
  })
  ...
  useEffect(() => {
    if (searchParams.location) {
      setSearchQuery(searchParams.location as string)
    }
  ...
  useEffect(() => {
    ...
    if (filters.bedrooms.length > 0) {
      results = results.filter((property) => property.bedrooms !== null && filters.bedrooms.includes(property.bedrooms))
    }
    ...
    if (searchQuery) {
      ...
      results = results.filter(
        (property) =>
          ...
          (property.neighborhood && property.neighborhood.toLowerCase().includes(query)) ||
  ...
  useEffect(() => {
    ...
    setTotalResults(initialProperties.length)
    setIsLoading(false)

    // Clear any existing compare properties when the page loads
  ...
  function sortProperties(props: Property[], option: string) {
    ...
    switch (option) {
      ...
      case "oldest":
        return sorted.sort((a, b) => new Date(a.date_added).getTime() - new Date(b.date_added).getTime())
      default:
        return sorted
    }
  }
  ...
  const nextImage = (propertyId: string, e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    // Since we only have one image per property in our database, this is a placeholder
    // In a real app, you would have multiple images per property
  }

  const prevImage = (propertyId: string, e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    // Since we only have one image per property in our database, this is a placeholder
    // In a real app, you would have multiple images per property
  ...
  const getCoordinates = (property: Property): [number, number] => {
    if (property.latitude && property.longitude) {
      return [property.latitude, property.longitude]
    }

    // Generate random coordinates around Zagreb, Croatia
    const centerLat = 45.815
    const centerLng = 15.982
                      ...
                      <div className="absolute inset-0 bg-[#e8e8e8]">
                        <div className="h-full w-full relative">
                          ...
                          {currentResults.slice(0, 8).map((property, index) => {
                            ...
                            const top = `${((45.9 - lat) / 0.2) * 100}%`
                            const left = `${((lng - 15.9) / 0.2) * 100}%`
                                ...
                                {hoveredProperty === property.id && (
                                  ...
                                  >
                                    <div className="relative h-32 w-full">
                                      <Image
                                        src={
                                          property.image_url || "/placeholder.svg?height=400&width=600&text=Nema+slike"
                                        }
                                        alt={property.title}
                                        fill
                          ...
                          <Link href={`/nekretnine/${property.id}`} key={property.id} className="block group">
                            <div className="flex gap-3 p-2 hover:bg-gray-50 rounded-lg transition-colors">
                              <div className="relative h-16 w-16 flex-shrink-0 rounded-md overflow-hidden">
                                <Image
                                  src={property.image_url || "/placeholder.svg?height=400&width=600&text=Nema+slike"}
                                  alt={property.title}
                                  fill
                                  className="object-cover"
                                />
...
function PropertyCard({
  ...
  prevImage: (id: string, e: React.MouseEvent) => void
  nextImage: (id: string, e: React.MouseEvent) => void
        ...
        <div className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 h-full">
          <div className="relative h-40 sm:h-48">
            ...
            <div className="relative w-full h-full overflow-hidden">
              ...
              {/* Navigation arrows - for future implementation with multiple images */}
              <button
        ...
        <div className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300">
          <div className="flex flex-col sm:flex-row">
            <div className="relative w-full sm:w-1/3 h-40 sm:h-48">
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-3 z-10 sm:hidden">
            ...
            <div className="p-3 sm:p-4 flex-1 flex flex-col">
              ...
              <div className="mt-auto flex justify-between items-center">
                ...
                <div className="text-xs text-gray-500 hidden sm:block">
                  Dodano: {new Date(property.date_added).toLocaleDateString("hr-HR")}
                </div>
              </div>
            </div>
          </div>
